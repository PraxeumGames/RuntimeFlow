...
public sealed class InitializationExecutionPolicyTests
{
    ...
    public async Task ExecuteAsync_PreCancelledToken_TaskIsCancelled()
    {
        ...
        await AsyncTestAssert.ThrowsAsync<OperationCanceledException>(async () => await task);
        Assert.That(task.IsCanceled, Is.True, "Task must be in Cancelled state, not Faulted.");
        Assert.That(called, Is.False, "Operation must not be invoked when the token is already cancelled.");
    }
