...
public sealed class RuntimePipelineLoadingProgressIntegrationTests
{
    ...
    public async Task RunFlow_ModuleLoadAndReload_EmitsMonotonicProgressSnapshots()
    {
        ...
        Assert.That(pipeline.GetRuntimeStatus().CurrentOperationCode, Is.EqualTo("run_flow"));
        ...
        Assert.That(moduleSnapshots, Is.Not.Empty);

        var groupedByOperation = moduleSnapshots
            .GroupBy(snapshot => snapshot.OperationId)
            .ToArray();

        Assert.That(groupedByOperation, Has.Length.EqualTo(2));
    ...
    public async Task LoadAndReloadModule_HeavyBindings_EmitMonotonicProgressPerOperation()
    {
        ...
        Assert.That(heavyService.Attempts, Is.EqualTo(2));
        ...
        {
            Assert.That(snapshots, Has.Length.GreaterThanOrEqualTo(6));
    ...
    public async Task ReloadModule_WithActivationHooks_EmitsActivationAndDeactivationSnapshots()
    {
        ...
        Assert.That(reloadSnapshots, Is.Not.Empty);
        Assert.That(
            reloadSnapshots.Any(snapshot => snapshot.Message?.Contains("deactivation", StringComparison.OrdinalIgnoreCase) == true),
            Is.True);
        Assert.That(
            reloadSnapshots.Any(snapshot => snapshot.Message?.Contains("activation", StringComparison.OrdinalIgnoreCase) == true),
            Is.True);
