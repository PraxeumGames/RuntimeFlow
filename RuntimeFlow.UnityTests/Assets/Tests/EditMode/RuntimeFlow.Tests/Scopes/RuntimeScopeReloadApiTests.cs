...
public sealed partial class RuntimeScopeReloadApiTests
{
    private static async Task AssertCanceledProgressForSessionReloadAsync()
    {
        ...
        await AsyncTestAssert.CatchAsync<OperationCanceledException>(() => reload);
        ...
        string expectedOperationIdPrefix)
    {
        ...
        Assert.IsNotNull(canceledSnapshot);
        Assert.That(canceledSnapshot!.OperationId.StartsWith(expectedOperationIdPrefix, StringComparison.Ordinal), Is.True);

        var operationSnapshots = snapshots
            .Where(snapshot => snapshot.OperationId == canceledSnapshot.OperationId)
            .ToArray();

        Assert.IsNotEmpty(operationSnapshots);
        Assert.AreEqual(RuntimeLoadingOperationStage.Preparing, operationSnapshots[0].Stage);
        Assert.AreEqual(RuntimeLoadingOperationStage.Canceled, operationSnapshots[operationSnapshots.Length - 1].Stage);
        Assert.AreEqual(RuntimeLoadingOperationState.Canceled, operationSnapshots[operationSnapshots.Length - 1].State);
    }
    ...
    private static void AssertScopeReloadProgressContract(IReadOnlyList<RuntimeLoadingOperationSnapshot> snapshots)
    {
        ...
        Assert.That(snapshots.Any(snapshot => snapshot.OperationKind == RuntimeLoadingOperationKind.RestartSession), Is.True);
        Assert.That(snapshots.Any(snapshot => snapshot.OperationKind == RuntimeLoadingOperationKind.ReloadScene), Is.True);
        Assert.That(snapshots.Any(snapshot => snapshot.OperationKind == RuntimeLoadingOperationKind.ReloadModule), Is.True);
        ...
        foreach (var operationSnapshots in snapshots.GroupBy(snapshot => snapshot.OperationId).Select(group => group.ToArray()))
        {
            ...
            Assert.That(operationSnapshots.All(snapshot => snapshot.OperationKind == operationKind), Is.True);
